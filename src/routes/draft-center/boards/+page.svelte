...
<div class="min-h-svh bg-offWhite px-4 pb-16">
	<div class="mx-auto flex max-w-screen-xl flex-col gap-8">
		<nav aria-label="Draft board navigation" class="flex flex-wrap items-center justify-between gap-3">
			...
			<a href="/draft-center/leagues" class={buttonOptions({ variant: 'outline', size: 'md' })}>Leagues</a>
		</nav>

		<section class="border-[5px] border-black bg-white p-5 shadow-section-shadow md:p-8">
			<div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
				<div>
					<p class="mb-2 inline-block border-[3px] border-black bg-primary px-3 py-1 text-sm font-black uppercase text-white">
						Draft Boards
					</p>
					<h1 class="text-4xl font-black uppercase leading-none md:text-6xl">Manage your boards</h1>
					<p class="mt-3 max-w-2xl text-lg font-semibold text-gray-700">
						Create multiple predictions, submit them before lock, and choose which board enters global or private leagues.
					</p>
				</div>
			</div>
		</section>
		...
		<section class="grid gap-8 lg:grid-cols-[0.75fr_1.25fr]">
			<form method="post" action="?/create" class="flex flex-col gap-4 border-[5px] border-black bg-white p-5 shadow-section-shadow">
				...
				<label class="flex flex-col gap-2 font-bold">
					...
					<input name="name" value={form?.name || ''} class="border-[3px] border-black px-3 py-2" placeholder="Upside Swing Board" />
				</label>
				{#if form?.createError}
			...
			{#if form?.duplicateError}
				<p class="border-[3px] border-black bg-red-100 p-3 font-bold text-red-700">{form.duplicateError}</p>
			{/if}
			{#if form?.setGlobalError}
				<p class="border-[3px] border-black bg-red-100 p-3 font-bold text-red-700">{form.setGlobalError}</p>
			{/if}
			{#if form?.deleteError}
					...
					{#each boards as board}
						...
						<Card class="bg-white shadow-brut-shadow-sm">
								...
								<div>
									<div class="flex flex-wrap items-center gap-2">
										...
										{#if board.id === data.selectedGlobalBoardId}
											<span class="border-[3px] border-black bg-accent px-2 py-1 text-xs font-black uppercase">Global</span>
										{/if}
										<span class="border-[3px] border-black px-2 py-1 text-xs font-black uppercase">{board.status}</span>
									</div>
									<p class="mt-1 font-semibold text-gray-700">
										{board.pickCount} pick{board.pickCount === 1 ? '' : 's'}
										{#if board.leagueEntryCount > 0}
											· Used in {board.leagueEntryCount} league{board.leagueEntryCount === 1 ? '' : 's'}
										{/if}
									</p>
								</div>
								<div class="flex flex-wrap items-center gap-2">
									<a class={buttonOptions({ variant: 'secondary', size: 'sm' })} href={`/draft-center?board=${board.id}`}>
										<Pencil class="mr-1 inline size-4" aria-hidden="true" />
										Edit Picks
									</a>
									<form method="post" action="?/duplicate" class="flex gap-2">
										<input type="hidden" name="sourceDraftBoardId" value={board.id} />
										<input type="hidden" name="name" value={`Copy of ${board.name}`} />
										<button class={buttonOptions({ variant: 'outline', size: 'sm' })} type="submit">
											Duplicate
										</button>
									</form>
									<form method="post" action="?/setGlobal">
										<input type="hidden" name="draftBoardId" value={board.id} />
										<button
											class={buttonOptions({ variant: 'primary', size: 'sm' })}
											type="submit"
											disabled={board.status !== 'submitted' || board.id === data.selectedGlobalBoardId}
										>
											Use Global
										</button>
									</form>
							...
							<div class="mt-5 grid gap-3 border-t-[3px] border-black pt-4 md:grid-cols-[1fr_auto]">
								...
								<form method="post" action="?/delete" onsubmit={(event) => confirmDelete(event, board.name)}>
									<input type="hidden" name="draftBoardId" value={board.id} />
									<button
										class={buttonOptions({
											variant:
