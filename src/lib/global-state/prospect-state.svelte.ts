...
class ProspectDraftSystem {
	...
	constructor(
		...
		initialDraftBoard: DraftBoardType[],
		emptyDraftBoard: DraftBoardType[]
	) {
	...
	get availableProspects(): Prospect[] {
		return this.prospects.filter((prospect) => prospect.id && !this.isDrafted(prospect.id));
	}
	...
	replaceDraftBoard(newDraftBoard: DraftBoardType[]) {
		this.draftBoard = newDraftBoard;
		this.temporaryDraftedIds.clear();
		this.undraftedProspectIds.clear();
		this.setPermanentDrafted(
			newDraftBoard.flatMap((draft) => (draft.prospect?.id ? [draft.prospect.id] : []))
		);
	}
	...
	addProspectToBoard(prospect: Prospect, position = 1) {
		...
		if (prospect.id && this.temporaryDraftedIds.has(prospect.id)) {
			...
			const currentPosition = this.draftBoard.findIndex(
		...
		toast.success(
			`With the ${ordinalNumbers(position)} overall pick, the ${this.draftBoard[position - 1].teamName} select ${prospect.name}!`,
			{
				duration: 5000
			}
		);
...
export function setDraftSystem(
	prospects: Prospect[],
